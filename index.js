...
const layersIdx = {
	...
	"xiameicunIdx": {
		"layers": [ '交通','生产贸易','古建筑','宗教','机构'],
		"legend": ["xiameicunLegend"],
		"others": [],
	},
	"economicCorridorIdx": {
		"layers": [ "中国一带一路", "欧亚经济联盟", "蒙古草原之路"],
...
function setLayerVisibility(classId, vtype) {
	...
	if (classId=="cultureIdx" ||classId=="economicCorridorIdx"||classId=="xiameicunIdx"){
		for (var i = 0; i < layers.length; i++) {
			try {
				if (vtype == "on") {
...
function addHeritagePointLayer() {
	...
	map.addLayer({
		...
		type: 'symbol',
		source: 'chayuan',
		filter: ['in', 'attribute','古道','桥','关隘','海关','码头','火车站'],
		layout: {
			visibility: "none",
	...
	map.addLayer({
		...
		type: 'symbol',
		source: 'chayuan',
		filter: ['in', 'attribute','茶园与村落','茶厂','商号'],
		layout: {
			visibility: "none",
			'icon-image': 'trade',
			'icon-size':0.12,
	...
	map.addLayer({
		...
		type: 'symbol',
		source: 'chayuan',
		filter: ['in', 'attribute','刻碑','集镇','古镇','宅院'],
		layout: {
			visibility: "none",
			'icon-image': 'building',
			'icon-size':0.08,
	...
	map.addLayer({
		...
		type: 'symbol',
		source: 'chayuan',
		filter: ['in', 'attribute','俄商办公楼','府衙','中国茶商会馆'],
		layout: {
			visibility: "none",
			'icon-image': 'institute',
			'icon-size':0.08,
			// 'icon-allow-overlap': true
		},
...
function createHeritageToggleButton() {
	const addHeritagePointLayerLayerIds = ['交通','生产贸易','古建筑','宗教','机构'];
...
function createEconomicCorridorToggleButton() {
	const toggleableLayerIds = [ '中国一带一路','欧亚经济联盟', '蒙古草原之路'];
	for (const id of toggleableLayerIds) {
		if (document.getElementById(id)) {
			continue;
		}
...
function addTeaProductionLayer(){
	map.addSource('teaRegion', {
		'type': 'geojson',
